...
/* 		$this->pa_rs()->get_upgrade_url();
		$this->pa_rs()->add_filter( 'connect_url', 'pa_rs_settings_url' );
		$this->pa_rs()->add_filter( 'after_skip_url', 'pa_rs_settings_url' );
		$this->pa_rs()->add_filter( 'after_connect_url', 'pa_rs_settings_url' );
		$this->pa_rs()->add_filter( 'after_pending_connect_url', 'pa_rs_settings_url' );
		$this->pa_rs()->add_action('after_uninstall', 'pa_rs_uninstall_cleanup'); */
	}
	...
	public function enqueue_styles() {
		wp_register_style( 'page-animation-admin.css',  plugin_dir_url( __FILE__ ) . 'css/page-animation-admin.css', array(), null, 'all' );
		...
		wp_enqueue_style( 'page-animation-admin.css' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/page-animation-admin.js', array( 'jquery' ), $this->version, false );
		$script_params = array(
		   'post_obj' => 'post',
		   'plugin_name' => $this->plugin_name,
		   'site_url' => site_url(),
		   'animation_admin_url' => plugin_dir_url( __FILE__ ) . 'css/page-animation-admin.css',
	...
	public function add_plugin_admin_menu() {
		...
		add_submenu_page( $this->plugin_name, 'Page Animation Menu Display', 'Animations', 'manage_options', $this->plugin_name.'-display', array($this, 'display_plugin_setup_page_display') );
		add_submenu_page( $this->plugin_name, 'Page Animation Settings', 'Settings', 'manage_options', $this->plugin_name.'-settings', array($this, 'display_plugin_setup_page_settings') );
		/* $this->pa_rs()->add_filter( 'connect_url', 'pa_rs_settings_url' );
		$this->pa_rs()->add_filter( 'after_skip_url', 'pa_rs_settings_url' );
		$this->pa_rs()->add_filter( 'after_connect_url', 'pa_rs_settings_url' );
		$this->pa_rs()->add_filter( 'after_pending_connect_url', 'pa_rs_settings_url' );
	...
	public function add_action_links( $links ) {
	   $settings_link = array(
		'<a href="' . admin_url( 'admin.php?page='.$this->plugin_name.'-display') . '">' . __('Settings', $this->plugin_name) . '</a>',
...
/* 	public function update_plug(){
    ...
    public function display_plugin_setup_page_display(){
        include_once( 'partials/page-animation-admin-display.php' );
    }
	public function display_plugin_setup_page_settings(){
        include_once( 'partials/page-animation-admin-settings.php' );
    }
	...
	public function query_string($id_name){
		...
		$allowed_roles = array('editor', 'administrator', 'author');$user = wp_get_current_user();
		$allowed_roles = array('editor', 'administrator', 'author');
		if ( $this->pa_rs()->is_plan('basic', true) ) {
			$premium = true;
		}else{
			$premium = false;
		}
														...
														if($select_animation == "custom"){
															...
															if($choose_event == "load")
															{
																...
																$query_string .= "setTimeout(function(){ $('" . $attr . "').removeClass('" . $select_animation . "'); }, " . $timeOut . ");";	
																	...
																	if (($premium) || (array_intersect($allowed_roles, $user->roles))) {
													...
													if(($id_name['func_name'] == "get_animation_db")){
														$query_string .= "<div id='group_" . $animations . "' class='group'> \n"
															. "<h3> " . ($animations + 1) . ")  On ". $choose_event . " attribute " . $attr ." will " . $select_animation . "\n"
																...
																. "</div>\n"
															. "</h3>\n"
															. "<fieldset id='page-animation_admin_effects_" . $animations . "' class='page-animation-admin-effects'>\n"
